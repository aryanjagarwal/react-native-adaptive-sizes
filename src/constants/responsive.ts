...
export const scale = (size: number): number =>
  (screenSize.width / guidelineBaseWidth) * size;

export const verticalScale = (size: number): number =>
  (screenSize.height / guidelineBaseHeight) * size;
...
export const wp = (percentage: number): number =>
  (percentage * screenSize.width) / 100;

export const hp = (percentage: number): number =>
  (percentage * screenSize.height) / 100;
...
export const fontScale = (size: number, respectAccessibility: boolean = true): number => {
  ...
  const densityAdjustment =
    screenSize.pixelRatio && screenSize.pixelRatio > 3 ? 0.95 : 1.0;
...
export const platformScale = <T>(values: {
  ios?: T;
  android?: T;
  web?: T;
  default: T;
